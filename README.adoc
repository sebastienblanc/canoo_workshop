...
* Start the WildFly Server : `WL_LOCATION/bin/standalone.sh -Djboss.socket.binding.port-offset=1`
...
Now go to the `installation` tab and grab the `json` content (format = `Keycloak OIDC JSON`).

Add this as content to a new `keycloak.json` file under `src/main/webapp/WEB-INF`.
